...
session="${OPENCLAW_INTERACTION_SESSION:-main}"
prompt_path="${workspace}/prompts/interaction-agent-startup.md"
compat_prompt_path="${workspace}/prompts/pi-liaison-startup.md"
init_script="${workspace}/scripts/init-working-group.sh"
check_secrets_script="${workspace}/scripts/check-secret-config.sh"
...
if [ -x "${init_script}" ]; then
  ...
  echo "Panel initializer not found." >&2
  exit 1
fi

if [ ! -f "${prompt_path}" ]; then
  prompt_path="${compat_prompt_path}"
fi
...
if [ "${OPENCLAW_INTERACTION_START_GATEWAY:-1}" != "0" ]; then
  ...
  sleep "${OPENCLAW_INTERACTION_GATEWAY_WAIT_SECONDS:-2}"
fi

if [ -n "${OPENCLAW_INTERACTION_AGENT_ID:-}" ] && openclaw agent --help 2>/dev/null | grep -q -- "--agent"; then
  exec openclaw agent \
    --agent "${OPENCLAW_INTERACTION_AGENT_ID}" \
    --session-id "${session}" \
    --message "${prompt}"
fi
...
Start OpenClaw normally, then begin with this prompt:
